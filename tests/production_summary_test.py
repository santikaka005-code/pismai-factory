...
def payload(fruit_type):
    return {
        ...
        "employees": [
            {"id": 1, "emp_code": "01", "fullname": "Employee One"},
            {"id": "2", "emp_code": "02", "fullname": "Employee Two"},
        ],
...
def detail_headers(workbook):
    return [cell.value for cell in workbook["รายละเอียด"][5]]
...
def test_excel_columns_follow_selected_fruit():
    ...
    assert isinstance(mangosteen_book["รายละเอียด"].cell(6, pile_column).value, int)
...
def test_employee_name_lookup_normalizes_ids_and_codes():
    data = payload("mangosteen")
    assert report_server.employee_name_for_record(data, {"employee_id": "1"}) == "Employee One"
    assert report_server.employee_name_for_record(data, {"employee_id": 2}) == "Employee Two"
    assert report_server.employee_name_for_record(data, {"emp_code": "01"}) == "Employee One"
    assert report_server.employee_name_for_record(data, {"employee_name": "Fallback Name"}) == "Fallback Name"


if __name__ == "__main__":
    test_fruit_filter_and_numeric_piles()
    test_excel_columns_follow_selected_fruit()
    test_employee_name_lookup_normalizes_ids_and_codes()
